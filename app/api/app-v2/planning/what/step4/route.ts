...
export async function POST(req: NextRequest) {
  try {
    ...
    const checkResult: { valid: boolean; response?: NextResponse } = await checkPageStep(sessionId, 'what_scratch', 4);
    ...
    return NextResponse.json({ message: 'Success', url: `${process.env.NEXT_PUBLIC_APP_V2_URL}/planning/how` }, { status: 200 });
  } catch (error) {
