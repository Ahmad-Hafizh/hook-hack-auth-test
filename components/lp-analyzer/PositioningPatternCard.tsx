...
const ValueItem: React.FC<ValueItemProps> = ({
  ...
  return (
    <div className="flex gap-3 pl-1">
      <div
        className={`w-1 ${borderColor} rounded-full shrink-0 h-auto self-stretch`}
      ></div>
      <div className="w-full">
        ...
        <textarea
          className="editable-input text-xs font-bold text-text-main leading-relaxed"
          rows={2}
          value={value}
...
export const PositioningPatternCard: React.FC<PositioningPatternCardProps> = ({
  ...
  const functionalIconColor = isSelected ? "text-blue-600" : "text-text-muted";
  const emotionalIconColor = isSelected ? "text-text-muted" : "text-text-muted";
  const functionalLabelColor = isSelected ? "text-blue-600" : "text-text-muted";
  const emotionalLabelColor = isSelected
    ? "text-text-muted"
    : "text-text-muted";

  return (
    <label className="relative cursor-pointer group flex flex-col h-full">
      <input
      ...
      <div className="flex flex-col h-full bg-white rounded-2xl border-2 transition-all duration-200 overflow-hidden border-border-light shadow-sm hover:shadow-lg peer-checked:border-blue-600 peer-checked:shadow-lg peer-checked:ring-4 peer-checked:ring-blue-50/50">
        <div className="p-6 flex flex-col h-full">
          ...
          <div className="bg-gray-50 rounded-lg p-4 mb-6 border border-gray-100">
            ...
            <textarea
              className="editable-input text-lg font-bold text-text-main leading-snug"
              rows={2}
              value={promise}
          ...
          <div className="mb-6">
            <div className="flex items-center gap-2 mb-3">
              <span
                className={`material-symbols-outlined ${functionalIconColor} text-[18px]`}
              >
                settings
              </span>
              <span className={`text-xs font-bold ${functionalLabelColor}`}>
                機能的価値
              </span>
            </div>
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-3">
              <span
                className={`material-symbols-outlined ${emotionalIconColor} text-[18px]`}
                style={{ fontVariationSettings: "'FILL' 1" }}
              >
                favorite
              </span>
              <span className={`text-xs font-bold ${emotionalLabelColor}`}>
                情緒的価値
              </span>
            </div>
            <div className="space-y-3">
