...
export async function POST(req: NextRequest) {
  try {
    ...
    if (data.status == 'done') {
      ...
      const selectedTemplateData = templatesCreatomateModification.find((t) => t.template_id === session?.planningPlans?.template_id);
      const { variants } = data.result;
      const { hooks, strong_point_1_messages, strong_point_2_messages, strong_point_3_messages, ctas } = variants;

      await prisma.planningVariants.create({
        data: {
      ...
      return NextResponse.json(
        {
          ...
          variants: {
            ...
            ctas: ctas,
            strong_point_1_images: selectedTemplateData?.images.strong_point_1 ? [selectedTemplateData.images.strong_point_1] : [],
            strong_point_2_images: selectedTemplateData?.images.strong_point_2 ? [selectedTemplateData.images.strong_point_2] : [],
            strong_point_3_images: selectedTemplateData?.images.strong_point_3 ? [selectedTemplateData.images.strong_point_3] : [],
            background_music: selectedTemplateData?.images.background_music ? [selectedTemplateData.images.background_music] : [],
      ...
      return NextResponse.json({ message: 'data is fetching...', status: 'running' }, { status: 202 });
    } else {
      return NextResponse.json({ message: 'Error in job processing', status: 'error' }, { status: 500 });
    }
  } catch (error) {
