...
const UploadImageButton: React.FC<UploadImageButtonProps> = ({ onUploadImage, children, disabled, aspectRatio }) => {
  ...
  const handleConfirm = React.useCallback(async () => {
    ...
    if (!aspectRatio || !croppedAreaPixels) {
      console.log('[UploadImageButton] No aspect ratio or crop area, uploading original file');
      uploadImage({
        file: sourceFile,
        onUploadImage,
      });
    ...
    try {
      ...
      const croppedFile = new File([blob], sourceFile.name || 'cropped-image.jpg', { type: blob.type || 'image/jpeg' });
      ...
      console.error('[UploadImageButton] Error while cropping & uploading', error);
    }
  ...
  const onDrop = React.useCallback(
    (acceptedFiles: File[]) => {
      ...
      if (!aspectRatio) {
        console.log('[UploadImageButton] No aspect ratio provided, uploading original file immediately');
        uploadImage({
          file,
          onUploadImage,
        });
  ...
  return (
    <Popover>
      ...
      <PopoverContent className="w-[360px] h-[420px] flex flex-col gap-3 p-3" onCloseAutoFocus={() => setImage('')}>
        {!showCropper && (
        ...
        {showCropper && (
          <div className="flex flex-col gap-3 w-full h-full">
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button size="sm" className="border-rose-600 bg-rose-600 hover:bg-rose-500 text-white" onClick={handleConfirm}>
                Save
              </Button>
            </div>
          </div>
