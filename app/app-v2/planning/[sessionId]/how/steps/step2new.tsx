...
export const Step2New = ({ onNext, onPrev }: { onNext: () => void; onPrev?: () => void }) => {
  const { onSetDataRows, jobId, dataRows, onSetFinalizedDataRows, duration, orientation } =
    useDataContext();
    ...
    setSelectedRow((prev: any) => {
      ...
      if (isCurrentTarget) {
        ...
        if (currentSelections.includes(rowIndex)) {
          ...
          return {
            ...prev,
            [field]: currentSelections.filter((idx: number) => idx !== rowIndex),
          };
        } else {
          // Add to selection
          return {
  ...
  const countTextPatterns = (): number => {
    ...
    if (hookCount === 0 || body1Count === 0 || body2Count === 0 || ctaCount === 0) {
      return 0;
    }
    ...
    return selectedRow[selectedTarget].length;
  };
  ...
  React.useEffect(() => {
    if (jobResult) {
      if (jobResult.status === "done" && jobResult.variants) {
        const hook_1 = jobResult.variants.hook_1 || [];
        const hook_2 = jobResult.variants.hook_2 || [];
        const body_a_1 = jobResult.variants.body_a_1 || [];
        const body_a_2 = jobResult.variants.body_a_2 || [];
        const body_b_1 = jobResult.variants.body_b_1 || [];
        const body_b_2 = jobResult.variants.body_b_2 || [];
        const cta_1 = jobResult.variants.cta_1 || [];
        const cta_2 = jobResult.variants.cta_2 || [];
        ...
        if (maxRows > 0) {
          const dataRows = Array.from({ length: maxRows }, (_, i) => ({
            ...
            body1: { part1: cleanText(body_a_1[i]), part2: cleanText(body_a_2[i]) },
            body2: { part1: cleanText(body_b_1[i]), part2: cleanText(body_b_2[i]) },
    ...
    const patternCombination = selectedRow[selectedTarget].map(
      (rowIndex: number) => {
        ...
        const newRow: IDataRowFinalized = {
          hookPart1: hookParts.part1 || "",
          hookPart2: hookParts.part2 || "",
          body1Part1: body1Parts.part1 || "",
          body1Part2: body1Parts.part2 || "",
          body2Part1: body2Parts.part1 || "",
          body2Part2: body2Parts.part2 || "",
          ctaPart1: ctaParts.part1 || "",
          ctaPart2: ctaParts.part2 || "",
          hookImage: "",
          body1Image: "",
          body1ImageB: "",
          body2Image: "",
      ...
      <div className="w-full bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        <table className="w-full border-collapse text-sm table-fixed">
          <thead className="z-50">
            <tr className="h-[54px] bg-slate-100 border-b border-slate-200 z-[80] sticky top-0">
              <th
