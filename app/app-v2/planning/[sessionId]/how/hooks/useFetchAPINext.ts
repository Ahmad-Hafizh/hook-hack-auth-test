...
export const uploadImage = async ({
  ...
  try {
    ...
    console.log("[uploadImage] Uploading file:", file.name, file.size);
    ...
    console.log("[uploadImage] API response:", data);

    // Handle different possible response structures
    const imageUrl = data.url || data.image_url || data.imageUrl || data.data?.url;

    if (imageUrl) {
      console.log("[uploadImage] Calling onUploadImage with URL:", imageUrl);
      onUploadImage(imageUrl);
      return imageUrl;
    } else {
      console.error("[uploadImage] No URL found in response:", data);
      return null;
    }
  } catch (error) {
