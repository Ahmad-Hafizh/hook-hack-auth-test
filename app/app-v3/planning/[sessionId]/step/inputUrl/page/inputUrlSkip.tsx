...
const InputUrlSkipPage = ({
  ...
  const onSubmit = async () => {
    try {
      ...
      const { data } = await callApi.post("/app-v3/planning/what/step1/skip", {
        sessionId,
        user_url: userUrl,
        competitor_urls: [competitor1, competitor2, competitor3],
      });
  ...
  const onRegenerate = async () => {
    try {
      ...
      const { data } = await callApi.post(
        "/app-v3/planning/what/step1/skip/fetch",
        {
          sessionId,
        },
      );
      ...
      if (data) {
        if (data.user_url) {
          setUserUrl(data.user_url);
        }
        if (data.competitor_urls) {
          setCompetitor1(data.competitor_urls[0] || "");
          setCompetitor2(data.competitor_urls[1] || "");
          setCompetitor3(data.competitor_urls[2] || "");
        }
      }
    } catch (error) {
      console.log(error);
  ...
  return (
    <>
      <LoadingOverlay
        ...
        message="URLを分析しています。しばらくお待ちください。"
        showProgress={false}
      />
...
export default InputUrlSkipPage;
