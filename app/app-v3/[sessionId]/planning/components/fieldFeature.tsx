...
const convertTopToApiType = (value: string): "functional" | "emotional" => {
  return value === "emotion" ? "emotional" : "functional";
};

const convertBottomToApiFocus = (value: string): "process" | "outcome" => {
  return value === "result" ? "outcome" : "process";
};
...
const FieldFeature = ({
  ...
  const effectiveBottomSelected = tags
    ? getBottomSelected(tags)
    : bottomSelected;

  const handleTopChange = (newValue: string) => {
    if (!newValue || !onTagsChange) return;
    onTagsChange({
      ...tags,
      type: convertTopToApiType(newValue),
    });
  };

  const handleBottomChange = (newValue: string) => {
    if (!newValue || !onTagsChange) return;
    onTagsChange({
      ...tags,
      focus: convertBottomToApiFocus(newValue),
    });
  };
  ...
  if (onChange) {
    ...
    return (
      <div className="flex flex-col gap-2">
        <div className="flex justify-between items-start mb-1 h-[18px]">
          <label className="text-sm font-bold text-text-main flex items-center gap-2 pt-1">
            {label}
          </label>
          ...
          <div className="flex flex-col items-end gap-1.5">
            ...
            <div className="flex gap-2">
              <div className="">
                <span className="text-[10px] rounded px-2 py-0.5 h-fit bg-slate-200 text-slate-600">
                  {effectiveTopSelected === "function" ? "機能" : "情緒"}
                </span>
              </div>
              <div className="">
                <span className="text-[10px] rounded px-2 py-0.5 h-fit bg-slate-200 text-slate-600">
                  {effectiveBottomSelected === "process" ? "プロセス" : "結果"}
                </span>
              </div>
            </div>
          </div>
        </div>
        <div className={`pt-2 ${inputHeight ? inputHeight : "h-[68px]"}`}>
