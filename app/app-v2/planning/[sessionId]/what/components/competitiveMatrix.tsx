...
type CompetitiveMatrixProps = {
  ...
  keymessage?: string;
  keymessageTags?: MessageTags;
  onSetStrongPoint?: (strongPoint: string, index: number) => void;
  strongPoint?: string[];
  strongPointsTagged?: StrongPointTagged[];
  cta?: string;
  ctaTags?: MessageTags;
  competitors?: any[];
};
        ...
        <div className="bg-white p-5 rounded-xl border shadow-sm flex flex-col gap-6 h-full min-h-[860px]">
          ...
          {Array.from({ length: 3 }).map((_, index) => (
            <FieldFeature
              ...
              tags={
                strongPointsTagged?.[index]
                  ? {
                      type: strongPointsTagged[index].type,
                      focus: strongPointsTagged[index].focus,
                    }
                  : undefined
              }
              placeholder={`例：他にはない○○機能で、あなたの△△を解決します。`}
          ...
          {competitors.map((competitor, index) => (
            ...
            >
              <FieldFeature
                ...
                inputHeight="h-[110px]"
                label="キーメッセージ"
                tags={competitor.key_message_tags}
              ...
              {competitor.strong_points.map(
                (strongPoint: string, i: number) => (
                  <FieldFeature
                    ...
                    label={`強み・特徴 ${i + 1}`}
                    tags={
                      competitor.strong_points_tagged?.[i]
                        ? {
                            type: competitor.strong_points_tagged[i].type,
                            focus: competitor.strong_points_tagged[i].focus,
                          }
                        : undefined
                    }
                    value={strongPoint}
              ...
              <FieldFeature
                isTextarea={false}
                label="CTA"
                tags={competitor.cta_tags}
                placeholder="例：今すぐ無料で始める"
                value={competitor.cta || ""}
              />
            </div>
          ))}
        </div>
